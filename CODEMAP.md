...
| **Bề rộng / responsive** | `globals.css` → `.app-shell` `.topbar` `.screen` | ba bậc: `460px` (điện thoại) → `sm:560px` → `lg:720px`, **có trần** vì cột chữ rộng quá ~70 ký tự là khó đọc · lưới lối tắt & thư viện ảnh `lg:grid-cols-3` · phần dư dùng để **thở** (padding, bo góc), không kéo giãn nội dung | thu cửa sổ qua 3 bậc; kiểm `@media(min-width:1024px)` có trong `.next/static/css/*.css` sau `npm run build` |
