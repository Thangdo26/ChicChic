...
`npm test` → [vitest.config.ts](vitest.config.ts) → `tests/*.test.ts`. **310 phép kiểm, ~3 giây**, chạy trong CI trước bước build.
...
- ⟹ Bộ này **không phủ cổng quyền, không phủ phép ghi DB, không phủ biên giới với dịch vụ ngoài**. Đừng đọc "310 passed" thành "an toàn để deploy".
