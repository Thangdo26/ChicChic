...
| [Chuong3D.tsx](src/components/Chuong3D.tsx) `95` | *(mặc định)* `Chuong3D` | Lớp bọc **client** mỏng quanh `Coop`: đúng hai `useState` (con đang chạm / con đang rê chuột) + một dòng chú thích dưới hình. Không Prisma, không `useEffect`, không vòng lặp vẽ |
