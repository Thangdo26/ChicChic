...
> **Trải nghiệm 09/09/2026:** parent tự xác nhận Family, scene động/yếm, admin catalog, proof snapshot DECOR/GEAR và các sửa race/ownership/history. Đọc [runbook trải nghiệm](docs/engineering/CC-EXPERIENCE-20260909.md); migration danh mục đi trước deploy.
...
**Delta 09/09:** `/admin` thêm DecorCatalogForm (isAdmin), `/gia-dinh` opt-in trực tiếp cho owner LAYER đủ điều kiện; `/be/[childId]` thêm FarmPlayground sau requireChildUser + moKhuCuaBe kiểm parent còn sở hữu barn. Hai trang nông dân gửi snapshot DECOR/GEAR đang xem. Không thêm route công khai.
...
| `/chuong/[id]/ket-chu-ky` | [page.tsx](src/app/chuong/[id]/ket-chu-ky/page.tsx) | ↑ | Flock + request gần nhất, trạng thái chờ/lý do từ chối, liên kết nhật ký proof; Family chỉ RETIRE; RENEW chưa mở | `actions.decideEndOfLay`, `actions.cancelLifecycleRequest` |
...
| **`/admin/gia-dinh`** | [page.tsx](src/app/admin/gia-dinh/page.tsx) | **`isAdmin()`** rồi **`batFamily()` → `notFound()`** (Basic Auth của middleware chỉ phủ `/admin/*`, nên route **phải** nằm dưới đó) | Số đếm theo **nhóm**, qua `van-hanh.bangVanHanh` - ⚠️ **không một cột nào lần ngược ra một đứa trẻ cụ thể** (§17.5) | **không có** - trang chỉ để nhìn; nút tạm dừng nằm ở khối 👨‍👩‍👧 trên `/admin` |
| **`/gia-dinh/de-xuat`** | [page.tsx](src/app/gia-dinh/de-xuat/page.tsx) | **`requireUser`** rồi **`batFamily()` → `notFound()`** | `ChildSuggestion` của tôi qua **`dsMongMuon`** (lọc kèm `parentId` trong `where`) + nhãn chuồng | `learning-actions.nhoCoChuLam/ghiNhoMongMuon/boQuaMongMuon` - ⚠️ **không nút nào ở đây mua gì**; món trang trí chỉ có một `Link` sang luồng cũ (§9.41) |
...
| `/be/[childId]/mong-muon` | như `/be/[childId]` | Bé nhắn một điều cho bố mẹ (Epic 6). Chọn từ **catalog đóng**, không gõ chữ, **không thấy giá gì cả**. Ghi đúng một dòng `PENDING` và **không có tác động thật nào** (§9.41) |

## 3. Bản đồ ghi - ai được đụng vào bảng nào

**Delta 09/09:** catalog-actions ghi DecorItem qua isAdmin/slug/version CAS. installDecor ghi BarnDecor + Task + Event id `idem_…` cùng tx; biên nhận này không được prune. Sửa decor khóa Barn, upsertTask cùng tx và vô hiệu photoUrl; giữ BarnMedia/nhật ký. confirmDecorPaid/confirmMarketPaid ghi task cùng payment. deleteBarn chỉ dọn chuồng chưa từng dùng dưới khóa, không xóa Flock/Bird/Health/Lot/tiền/lịch sử.
...
| `BarnTask` (tạo/gộp) | `lib/task-store.upsertTask` cho việc thường; **`lib/lifecycle-store.createLifecycleRequest(tx)`** tạo riêng HARVEST/RETIRE gắn `lifecycleRequestId @unique`; không gộp vào việc thường | action kiểm quyền; request + task + nhật ký trong cùng transaction |
| `BarnTask.status` | `completeTask`/`declineTask` CAS `OPEN`; `cancelLifecycleRequest` đóng task kèm request; `cancelTask` chỉ xóa việc thường | đúng actor; lifecycle không được đi qua cửa xóa task thường |
| `FarmUpdate` `BarnMedia` | `worker-actions.*` (nông dân) · `actions.addMedia/stamp` (admin) | `activeWorkerSession()` / **`isAdmin()`** |
| **`HarvestLot`** | **chỉ** `worker-actions.logHarvest` | worker đúng chuồng + exact `flockId` + proof. MEAT cần đúng request đã ACCEPTED, đối soát cả đàn/count/weight/HealthEvent; `lifecycleRequestId @unique` chống trùng. EGG giữ cửa sổ 60s cũ |
...
| `Flock`/`Bird` terminal + **`FlockOutcome`** | `worker-actions.completeTask` → `completeLifecycle(tx)` | request đã được farm nhận, exact danh sách Bird và proof; MEAT có lô riêng đúng request + count/weight. CAS version/policy, tạo outcome và DONE cùng transaction; không ghi đè DECEASED |
| **`LifecycleRequest`** | `decideEndOfLay` → `createLifecycleRequest(tx)`; `acceptLifecycleTask`, `logHarvest`, `completeTask`, `declineTask`, `cancelLifecycleRequest` → helper trong `lifecycle-store` | unique actor/key và activeFlockId; request không đổi physical stage; RENEW bị từ chối |
| `LifecycleDecision` | giữ nguyên lịch sử cũ, không ghi mới trong CC-B01 | không suy đoán request/outcome từ decision cũ |
| `Flock.stage` → `END_OF_LAY` (admin) | `actions.setEndOfLay` | admin + CAS stage/version; không đặt lại đàn đã terminal |
...
| **`FamilyEnrollment.status`** (tạm dừng / mở lại) | **chỉ** [family-admin-actions](src/app/family-admin-actions.ts) `tamDungSuat`/`moLaiSuat` (§9.42) | `isAdmin()` + `batFamily()` **trước** mọi phép đọc · so-sánh-rồi-đặt mang trạng thái cũ trong `WHERE` (§9.24) · lý do lấy từ catalog **đóng** `LY_DO_TAM_DUNG` · ⚠️⚠️ **KHÔNG đụng `Flock.lifecyclePolicy`, `BarnTask`, `Barn.workerId`, `ChildProfile`, `DomainEvent`** - tắt trải nghiệm số không dừng chăm gà (§22.3 của spec) |
| **`BarnTask.careTag`** | **chỉ** [worker-actions.completeTask](src/app/worker-actions.ts) | Nhãn một chạm của nông dân (§18.3 · FL-D24). Khoá **đóng** trong `NHAN_CHAM_SOC`, lọc bằng `locNhan(kind, …)` · ⚠️ khoá lệch loại việc ⟹ **bỏ nhãn**, KHÔNG từ chối cả việc · **không bao giờ bắt buộc** |
| ⭐ **`ChildSuggestion`** (tạo) | **chỉ** [lib/de-xuat.ts](src/lib/de-xuat.ts) `taoMongMuon` ← `learning-actions.guiMongMuon` (§9.41) | ⚠️⚠️ **Dòng ở đây KHÔNG có tác động thật nào** - không việc, không đơn, không tiền (FL-D06/D07). `status` **không bao giờ** được đặt tay lúc tạo: mặc định của DB là `PENDING` · catalog đóng ở `de-xuat-meta` tra **trước** phép ghi · trần `TRAN_DANG_CHO_MOI_BE` mỗi bé và **`TRAN_CARE_WISH_TUAN` mỗi chuồng mỗi 7 ngày** (FL-D23: mỗi cái được duyệt là công của một người thật) · **không cột nào nhận chữ tự do của trẻ** |
| ⭐ **`ChildSuggestion.status`** (đổi trạng thái) | **chỉ** [learning-actions.ts](src/app/learning-actions.ts) `nhoCoChuLam`/`ghiNhoMongMuon`/`boQuaMongMuon` · `EXPIRED`: [lib/de-xuat.hetHanMongMuon](src/lib/de-xuat.ts) ← `jobs` | Cổng **`moMongMuon`**: sở hữu hồ sơ **và** sở hữu chuồng (FL-D22) · so-sánh-rồi-đặt kèm `parentId` · ⚠️ cổng **không lọc sẵn `status`** - bấm lại thứ mình vừa trả lời phải nhận một câu **tử tế**, không phải "không tìm thấy" |
...
**Module thêm 09/09:** lib/decor-catalog (16 mẫu/validate), app/catalog-actions (saveDecorCatalog), DecorCatalogForm, lib/decor-proof (decorProofSnapshot), components/be/FarmPlayground, check-catalog-schema.cjs, render-coop-preview.tsx, tests/experience.test.tsx. nhanLoiMoiGiaDinh nhận enrollmentId **hoặc barnSlug**, cần childId/xacNhan; không bắt buộc lời mời admin.
...
| [van-hanh-meta.ts](src/lib/van-hanh-meta.ts) | **`LY_DO_TAM_DUNG`** `timLyDoTamDung` `cauTamDung` (catalog **đóng**, chữ cha mẹ đọc) · **`NHAN_CHAM_SOC`** `nhanChoViec` **`locNhan`** (5 nhãn của §18.3) · **`tiLe`** `soNguong` `soTran` **`NGUONG_PILOT`** `PHUT_MOI_VIEC_UOC` `CANH_BAO_UOC` · `CHU_DAN_XUAT` `tenTepXuat` - **thuần** | family-admin-actions · worker-actions · `FamilyPilotForms` · `WorkerForms` · `/gia-dinh` · `/admin/gia-dinh` · van-hanh.ts · xuat-du-lieu.ts · tests/van-hanh |
| [van-hanh.ts](src/lib/van-hanh.ts) | **`bangVanHanh`** - bảng cohort + 5 chỉ số §4.4 + SLA dữ liệu. ⚠️ **CHỈ ĐỌC** (bộ kiểm đếm lời gọi ghi = 0) · tỉ lệ trả **`null`** khi mẫu số 0, không trả 0% · cỡ pilot 10–15 nhà nên cố ý lấy cả bảng rồi đếm trong bộ nhớ | `/admin/gia-dinh` |
| [xuat-du-lieu.ts](src/lib/xuat-du-lieu.ts) | **`goiDuLieuTre`** - gói toàn bộ dữ liệu **một** bé để cha mẹ tải về (§17.3 mục 5). ⚠️ **CHỈ ĐỌC**, mọi truy vấn lọc theo đúng `childId` (bộ kiểm đếm: số truy vấn = số phép lọc) · **không đường dẫn ảnh/video** - ảnh là dữ liệu của nông trại | family-actions.taiDuLieuTre |
| [de-xuat-meta.ts](src/lib/de-xuat-meta.ts) | **`MONG_MUON`** (catalog **đóng**, 4 loại × 4–5 lựa chọn) `timMongMuon` `nhomMongMuon` `NHAN_NHOM` **`TRAN_CARE_WISH_TUAN`** `TRAN_DANG_CHO_MOI_BE` `NGAY_HET_HAN_MONG_MUON` - **thuần**, và **không một chữ nào về tiền** trong thứ bé đọc | de-xuat.ts · `WishPicker` · `/gia-dinh/de-xuat` · tests/de-xuat |
| [de-xuat.ts](src/lib/de-xuat.ts) | ⭐ **`taoMongMuon`** - **đường DUY NHẤT ghi `ChildSuggestion`** (§9.41), luôn `PENDING` · ⭐ **`moMongMuon`** - **cổng của mọi hành động cha mẹ**: sở hữu hồ sơ **và** sở hữu chuồng · `khoaDangCho` `demMongMuonCho` `dsMongMuon` · `hetHanMongMuon` (việc nền, im lặng) | learning-actions · `/be/[childId]/mong-muon` · `/gia-dinh` · `/gia-dinh/de-xuat` · jobs |
...
| [bai-hoc-meta.ts](src/lib/bai-hoc-meta.ts) | **`CATALOG`** (6 chương × 2 nhóm tuổi = 12 đơn vị) `chonDonVi` (chọn bài hoặc **lý do bỏ qua**) **`TU_CAM_NOI_DUNG`** `chuCuaTre` (mọi chữ trẻ đọc - cho bộ kiểm) `chupNoiDung` (sao **sâu**) **`TRUONG_DU_KIEN`** `locDuKien` · Epic 6: **`TU_CAM_BAO_CAO`** `cauTuanNay` `cauChuongTuan` (trả **`null`** khi không có gì để nói) - **thuần** | bai-hoc.ts · jobs.ts · `/gia-dinh` · tests/bai-hoc + tests/de-xuat |
| [bai-hoc.ts](src/lib/bai-hoc.ts) | ⭐ **`moKhuCuaBe`** - **cổng DUY NHẤT của khu trẻ em** (§9.40), 5 điều kiện, đọc lại DB **mỗi lần** · **`moBaiCuaBe`** - thêm phép so "bài này của đúng bé này" · **`anhCuaBai`** - đổi id ảnh thành đường dẫn, **lọc theo `barnId`** · ⭐ **`dungKhoanhKhac`** - **đường DUY NHẤT sinh bài** (§9.39) · **`demBaiDangCho`** - chỉ ĐẾM, dùng để vẽ · **`baoCaoTuan`** - "tuần này con đã khám phá gì" (Epic 6), **chỉ đọc**, không xếp hạng | learning-actions · jobs (`bai-hoc-cho-be`) · `/gia-dinh` (chỉ `demBaiDangCho` + `baoCaoTuan`) |
...
| [chuong-3d.ts](src/lib/chuong-3d.ts) `260` | ⭐⭐ **`ganYem`** - cửa DUY NHẤT dựng `GaVM` (§9.43) · `viTriDan` `DAN_TOI_DA` `cauSoCon` · `tenGa` `cauGa` · `hopMat` `matKhoi` `sang` `toi` `KHUNG` | **Thuần** - không Prisma/env/react, nạp được vào cả server, client lẫn vitest. Hệ toạ độ **giữ nguyên 240×180** vì mỗi dòng `BarnDecor.x/y` là bản vẽ cô chú lắp thật |
...
| [cache.ts](src/lib/cache.ts) `68` | **`cachedDecorItems`** `cachedZones` `cachedBreed` `cachedFeedingPlan` (TTL 1 giờ; danh mục decor do admin ghi và revalidate tag `catalog`, giữ cả món ngừng bán cho kho đã mua) · **`cachedFarmProof`** (số liệu trang chủ, TTL 5 phút) | `/`, `/nhan-chuong`, `/trang-tri`, `api/reservations` |
...
| | *(cả nhóm decor)* | | `mutateDecor`/`installDecor` khóa Barn, ghi bản vẽ + `upsertTask(DECOR)` cùng transaction; sửa bản vẽ xóa xác nhận ảnh cũ |
...
| | **`removeGear(slug, gearId)`** | chủ chuồng | `PENDING_ON` (chưa mặc thật) → `OFF`, giữ dòng/removedAt; yếm về kho ngay, đóng GEAR OPEN nếu không còn yêu cầu · `WORN` → `PENDING_OFF` + việc GEAR |
...
| | `decideEndOfLay(formData)` | chủ chuồng/admin qua `ownedBarn` | barn/flockId/expectedVersion/key/choice; policy kiểm lại dưới khóa; tạo REQUESTED + task; trả requestId/taskId; retry trả bản cũ |
| | `cancelLifecycleRequest(slug, requestId)` | `ownedBarn` + đúng owner snapshot | chỉ REQUESTED; CAS sang CANCELLED + đóng task + nhật ký; không xóa request |
| [task-actions.ts](src/app/task-actions.ts) `79` | `requestTask(slug, kind, note, dueAtIso)` | chủ chuồng/admin | trần `MAX_OPEN_PER_BARN = 6` |
| | `cancelTask(taskId)` | chủ chuồng | chỉ việc thường OPEN; lifecycle phải rút qua `cancelLifecycleRequest` |
| [worker-actions.ts](src/app/worker-actions.ts) `170` | `markTasksSeen()` | nông dân | xoá dấu "MỚI" |
| | **`completeTask(taskId, formData)`** | nông dân đúng việc | CAS OPEN trước side effect, DONE/media/nhật ký/event atomic. Lifecycle kiểm request/proof/count/lô trong tx, đổi stage/Bird và tạo outcome; DONE replay trả ok |
| | `acceptLifecycleTask(taskId)` | `activeWorkerSession` + đúng task/chuồng | REQUESTED → ACCEPTED; ghi worker/time, RETIRE là farm nhận chăm tiếp; không đổi stage |
| | `declineTask(taskId, reason)` | nông dân đúng việc | lý do ≥5 ký tự; CAS task/request + nhật ký. Không từ chối khi MEAT đã có lô; cần đối soát |
| | `postDailyUpdate(formData)` | nông dân đúng chuồng | **không cần việc** - vòng lặp giữ chân |
| | **`logHarvest(formData)`** | nông dân đúng chuồng | EGG không cần task; MEAT cần flockId/requestId đã nhận, proof/count/weight và kiểm HealthEvent; một lô mỗi request, sang IN_PROGRESS, **chưa terminal** |
...
| [learning-actions.ts](src/app/learning-actions.ts) | `dongBoKhoanhKhac()` - **không nhận id nào từ client** · `batDauBai` `xongBai` `xongNhiemVu` (khu của bé, §9.40) · `moCuaRaNgoai` - cổng ra, **không** đóng dấu `reauthAt` · Epic 6: **`guiMongMuon({childId, optionKey})`** - hành động **của trẻ**, ghi một dòng `PENDING` và **không gì khác** · **`nhoCoChuLam({id})`** - hành động **của cha mẹ**, cửa DUY NHẤT biến mong muốn thành `BarnTask` thật (FL-D22) · `ghiNhoMongMuon` `boQuaMongMuon` - trả lời mà **không kéo theo hành động nào** | `MomentPlayer` · `ExitGate` · `WishPicker` · `DeXuatCard` · `LearningSyncButton` |
| [family-admin-actions.ts](src/app/family-admin-actions.ts) | **`tamDungSuat({enrollmentId, lyDo})`** · **`moLaiSuat({enrollmentId})`** | **`isAdmin()`** rồi **`batFamily()`** | Kill switch mức **một suất** (§22.3 · §9.42). `ACTIVE ↔ PAUSED`, so-sánh-rồi-đặt. ⚠️ **Không đụng đàn, việc, consent** - khu của bé đóng lại **nhờ cổng cũ** `moKhuCuaBe` vốn đòi suất `ACTIVE`, không nhờ dòng nào ở đây · cha mẹ nhận chuông mang **đúng câu người trực đã chọn** · `pauseReason` giữ lại sau khi mở lại (lịch sử vận hành) |
...
| [family-actions.ts](src/app/family-actions.ts) | **`taiDuLieuTre({childId})`** | đăng nhập + `batFamily()` + **`daXacMinhGanDay()`** + `canParentManageChild` | Cha mẹ tải toàn bộ dữ liệu của bé về máy (§17.3 mục 5 · §9.42). Trả **chữ**, client tự dựng tệp - không mở thêm route tải nào. ⚠️ **Cố ý KHÔNG gọi `xoaDauXacMinh()`** (khác rút/xoá): đường đi thật là *tải rồi mới xoá*, và bắt gõ mật khẩu hai lần trong một phút không làm ai an toàn hơn. Hồ sơ **đã rút** vẫn tải được; chỉ `DELETED`/`DELETION_PENDING` mới đóng |
...
| | **`nhanLoiMoiGiaDinh({enrollmentId?, barnSlug?, childId, xacNhan})`** | `parentId === me` **và** `barn.ownerId === me` + hồ sơ bé `ACTIVE` | ⚠️⚠️ **ĐƯỜNG DUY NHẤT GHI `Flock.lifecyclePolicy`** (§9.37). Một transaction khóa Barn → Flock → Enrollment → Child: tạo suất ACTIVE trực tiếp hoặc CAS INVITED→ACTIVE, link con và `DomainEvent.FAMILY_ENROLLED`, khóa policy đàn. Retry không ghi thêm; suất ACTIVE được thêm anh chị em. Không mở lại PAUSED. `xacNhan` là cam kết, **không phải cổng quyền**; materialize bài chào sau POST |
...
| [Illustrations.tsx](src/components/Illustrations.tsx) `530` | `Coop` `CoopBackdrop` **`DanGaKhoi`** **`GaKhoi`** `DecorSprite` `DecorFigure` `FarmerAvatar` `COOP_VIEWBOX` | SVG thuần, **không state, không `use client`** - 7 chỗ dùng nó, phần lớn là ảnh nhỏ 86px. Cảnh chuồng dựng bằng **khối hộp ba mặt** (§9.43); hình học nằm ở `lib/chuong-3d`, ở đây chỉ vẽ. ⚠️ `DanGaKhoi` vẽ **sau** decor trong `Coop` (để chạm được vào con gà) nhưng **trước** decor trong Decor Studio (ở đó thứ phải bắt cú chạm là món đang kéo) |
| [Chuong3D.tsx](src/components/Chuong3D.tsx) | client wrapper quanh Coop | State chọn/rê/dừng; cảnh lớn animated, aria-live, pause/reduced motion; không Prisma/RAF |
...
| [EndOfLayChoices.tsx](src/components/EndOfLayChoices.tsx) | flockId/version/request/duocChon | gửi intent với key giữ khi retry; RETIRE xác nhận điều khoản hiện có; status/reason/rút trước ACCEPTED; không mở RENEW, không khẳng định đã sơ chế/kiểm dịch |
...
```
Các đường khác cũng gộp `upsertTask`: `toggleRange` (RANGE_OUT/RANGE_IN); nhóm decor dùng `mutateDecor`/`installDecor` ghi bản vẽ và DECOR trong cùng transaction.
...
lắp:   installDecor(slug, itemSlug, requestId)      ├ barnActivated()  ├ free > 0  ├ tổng < MAX_DECOR_PER_BARN
...
xếp:   saveDecorLayout(slug, layout[])
          ...
          └ mutateDecor() → ghi bản vẽ + 1 việc DECOR trong cùng transaction
...
### 7.11 CC-B01: request → proof → physical outcome

```text
/ket-chu-ky → decideEndOfLay(barn, flockId, expectedVersion, key, choice)
  ownedBarn → transaction khóa Barn/Flock, đọc lại policy/version
  → LifecycleRequest REQUESTED + task HARVEST/RETIRE + nhật ký ý định
  → Flock vẫn END_OF_LAY, Bird không đổi; notification chỉ sau commit

acceptLifecycleTask → ACCEPTED + acceptedByWorkerId/acceptedAt
  RETIRE: farm nhận tiếp tục chăm; chủ đã xác nhận snapshot phí/điều khoản
  MEAT: kiểm các HealthEvent còn vấn đề/ngừng thuốc + Bird đang bệnh

MEAT: logHarvest(exact flockId/requestId, count, weight, proof lúc cân)
  → một HarvestLot liên kết request, status IN_PROGRESS; stage chưa đổi
completeTask(taskId, proof hoàn tất, confirmedCount)
  → CAS OPEN + proof + count khớp đúng danh sách Bird đã chụp
  → MEAT: exact lot/count/weight/proof + kiểm sức khỏe lần nữa
  → RETIRE: điều khoản chủ đã nhận + farm acceptance
  → Flock/Bird terminal + FlockOutcome + request COMPLETED + task DONE + event
     tất cả cùng transaction; lỗi ở bất kỳ bước nào → rollback toàn bộ
```

Một request hoạt động mỗi đàn (activeFlockId unique), một task và một lô mỗi request.
Idempotency khóa theo actor/key, replay trả kết quả cũ, key khác không chen được vào request đang hoạt động.
Chủ rút khi REQUESTED → CANCELLED; farmer từ chối trước khi ghi lô → DECLINED, kèm reason/next action.
Không lấy lô theo barn/ngày. Task HARVEST cũ thiếu liên kết phải đối soát, không tự gắn request hay đoán proof.
RENEW bị khóa tới CC-B02; không reset Flock/Bird/Product, không tạo lứa/hóa đơn mới.
Migration và giới hạn: [runbook CC-B01](docs/engineering/CC-B01-LIFECYCLE.md).
...
```
/ket-chu-ky → decideEndOfLay(RETIRE): REQUESTED + điều khoản phí hiện có
   ├ acceptLifecycleTask: farm nhận tiếp tục chăm
   ├ completeTask + proof/count → Bird/Flock RETIRED + outcome
   └ chủ chuồng → /chuong/<slug>/nghi-huu     ← lối đi tiếp, KHÔNG tự tạo hoá đơn
...
**Trải nghiệm 09/09:** đổi Family opt-in → family-actions/gia-dinh/family.ts/FamilyInviteCard/bai-hoc + PG/HTTP; đổi vật phẩm → decor-catalog/catalog-actions/DecorCatalogForm/DecorStudio/Illustrations/schema/gate/seed; đổi proof → decor-proof/WorkerForms/hai trang nông dân/completeTask; đổi capacity → API reservations và reassignBarn (cùng khóa FarmWorker). [runbook trải nghiệm](docs/engineering/CC-EXPERIENCE-20260909.md).
...
| Đổi **chỗ chốt "đàn đã mổ xong"** | `lifecycle-store.completeLifecycle` trong `worker-actions.completeTask` | exact request/flock/Bird snapshot + lot/count/weight/proof; CAS và event cùng tx; không dùng barn/ngày | `npm run test:lifecycle:pg`: 25 lần song song, sai target/thiếu proof/rollback |
...
| **Tạm dừng** một suất, hoặc thêm một lý do tạm dừng | `lib/van-hanh-meta.ts:LY_DO_TAM_DUNG` + `family-admin-actions.tamDungSuat/moLaiSuat` + `FamilyPilotForms` + khối cảnh báo ở `/gia-dinh` + `tests/van-hanh.test.ts` | ⚠️ **Đọc §9.42 TRƯỚC.** Bốn thứ tắt trải nghiệm số **không được** đụng (§22.3): chăm gà · sự kiện nông trại · `Flock.lifecyclePolicy` · consent. Thêm một lý do thì câu `choChaMe` phải (a) **nói tới đàn gà** - đó là câu gia đình hỏi đầu tiên, và (b) **không một chữ nào** về bệnh tật hay cái chết (§18.4). `/admin` và `/gia-dinh` phải hiện **cùng một câu**, nếu không thì lúc gia đình gọi điện, người trực đi đoán mình đã nói gì | `npm test` (bộ `van-hanh`) · rồi chạy thật: dừng một suất ⟹ khu của bé đóng, mà `Flock`, `BarnTask`, `ChildProfile.status` **không đổi một dòng nào** |
| Thêm/sửa **nhãn một chạm** của nông dân | `lib/van-hanh-meta.ts:NHAN_CHAM_SOC` + `worker-actions.completeTask` + `WorkerForms` + `su-kien-meta.TRUONG_PAYLOAD` | ⚠️ **Nhãn KHÔNG BAO GIỜ được thành bắt buộc** (mục tiêu §4: cô chú ≤30 phút/nông trại/tuần) - không nhánh `nope()` nào được nhìn tới nó, nút gửi cũng không · nhãn **thay** câu ghi chú mặc định, nên nó *bớt* gõ chứ không thêm việc · vào `payload` dưới dạng **khoá đóng**, không phải câu chữ (§9.38) · ⚠️ đổi nội dung bài học theo nhãn = viết biến thể mới ⟹ **phải qua chuyên gia giáo dục** (NO-GO §23), và bộ kiểm chặn `chonDonVi` đọc tới `tag` | `npm test` · rồi chạy thật: nhãn lệch loại việc ⟹ việc **vẫn xong**, nhãn bị bỏ |
| Thêm một **chỉ số pilot** vào `/admin/gia-dinh` | `lib/van-hanh.ts:bangVanHanh` + `van-hanh-meta.NGUONG_PILOT` + trang | ⚠️ **Mẫu số 0 ⟹ `null`, không phải 0%** (`tiLe`) - "0% kích hoạt" khi chưa mời ai là lời nói dối kinh điển của bảng số liệu tự làm · con số **ước** phải kèm `CANH_BAO_UOC` · **không cột nào** lần ngược ra một đứa trẻ cụ thể (§17.5) · trang **chỉ đọc**, không nút | `npm test` (bộ `van-hanh` đếm lời gọi ghi = 0) |
...
| Đổi **luật kết chu kỳ** / thêm chính sách | `family-gates.allowedLifecycleChoices` + `lifecycle.assertLifecycleChoice` + `lifecycle-store` | Family RETIRE-only ở request/log/complete; `family-actions` vẫn là cửa duy nhất ghi policy, cùng khóa Flock; STANDARD cũng chưa mở RENEW | unit + Postgres crafted POST/race Family acceptance |
...
| Sửa **hình chuồng** / thêm khối vào cảnh | `lib/chuong-3d.ts` (hình học) + `Illustrations.tsx` (nét vẽ) + `tests/chuong-3d.test.ts` | ⚠️ **Đọc §9.43 TRƯỚC.** Ba thứ không được nới: ① hệ toạ độ vẫn **240×180** - đổi là làm lệch mọi `BarnDecor.x/y` đã lưu, mà đó là bản vẽ cô chú lắp thật ngoài chuồng · ② **không thư viện 3D, không `<canvas>`, không `requestAnimationFrame`** - trang chuồng là trang nặng nhất của app (§11.23) và người ta mở nó bằng điện thoại giữa đồng · ③ `Illustrations.tsx` **không được có `use client`**: 7 chỗ dùng nó, phần lớn là ảnh nhỏ trong danh sách | `npm test` (bộ `chuong-3d`) · rồi **nhìn bằng mắt**: mở `/chuong/<slug>` và đếm số gà trên hình so với ô "Đàn · N con" |
| Đổi **số con vẽ trên hình chuồng** | mọi chỗ gọi `<Coop>` phải truyền `soCon` (đếm) hoặc `dan` (có tên) | ⚠️⚠️ **Số con lấy từ `Bird` đang `ALIVE`, KHÔNG lấy `flock.size`.** `size` là số chủ chuồng chọn lúc nhận nuôi; đàn đã nhận thịt / đã nghỉ hưu thì `size` vẫn nguyên mà ngoài vườn không còn con nào - vẽ 6 con vào cái sân đã trống là kiểu sai tệ nhất, nó trông y như đúng. Danh sách nhiều chuồng thì dùng **một `bird.groupBy`**, không phải một câu mỗi chuồng (§10) | `npm test` · rồi mở `/chuong`, `/tai-khoan`, `/nong-trai` xem ảnh nhỏ có đổi theo không |
| Cho **con gà trên hình** mặc thêm thứ gì | `lib/chuong-3d.ts:ganYem` + `Illustrations.GaKhoi` | ⚠️⚠️ **Đọc §9.43.** Món đó chỉ được vẽ khi nó đã ở trên con vật THẬT (§9.1) - tức là phải đi qua `ganYem`, và trạng thái "chủ chuồng đã chọn, cô chú chưa làm" thì trả `null` kèm một câu vào `cho`. **Đừng** truyền `BirdGear.status`/`colorHex` xuống tầng vẽ: tầng vẽ mà biết trạng thái thì sớm muộn có người vẽ theo | `npm test` · rồi chạy thật: bấm mặc yếm ⟹ hình **không đổi**, chỉ hiện dấu chờ; cô chú báo xong kèm ảnh ⟹ hình mới đổi |
| Thêm **món decor** | `/admin#danh-muc-trang-tri` → `saveDecorCatalog` | Chọn một trong 16 mẫu đóng. Thêm mẫu mới bằng code thì cập nhật `decor-catalog.ts`, `DecorSprite` và test render | thử tạo/ngừng bán; món đã mua và giá đơn cũ giữ nguyên |
| Thêm **màu yếm** | Admin chọn mẫu `yem`, nhập màu `#RRGGBB`, tạo mã món mới | Không migrate; hình/loại/màu bất biến sau tạo để không đổi món khách đã mua. Seed chỉ thêm mã còn thiếu | mua → yêu cầu mặc → proof → kiểm màu trong cảnh và kho |
...
| **Xóa chuồng** | admin-actions.deleteBarn + BarnDeleteForm | Chỉ chuồng chưa từng dùng; FOR UPDATE Barn, đếm mọi quan hệ, Flock/reservation/owner/Event phải rỗng | PG có lịch sử → từ chối, mọi dòng còn nguyên; chuồng trống → dọn, retry vô hại |
...
| Thêm một **mong muốn** vào catalog của bé, hoặc đổi luật duyệt | `lib/de-xuat-meta.ts:MONG_MUON` + `lib/de-xuat.ts` + `learning-actions` + `/gia-dinh/de-xuat` + `tests/de-xuat.test.ts` | ⚠️ **Đọc §9.41 TRƯỚC.** Thêm một dòng thì phải trả lời được **"cha mẹ bấm đồng ý rồi thì SAO?"** - không trả lời được là đang thêm một nút không dẫn tới đâu, và ở đây nó tệ hơn nút chết thường vì đứa trẻ đã gửi đi một điều nó thật sự mong · `CARE_WISH` thì mỗi dòng là **công của một người thật** (trần `TRAN_CARE_WISH_TUAN`, mục tiêu §4 của spec là ≤30 phút/nông trại/tuần) · `DECOR_WISH` phải trỏ tới một `slug` **có thật** trong `DECOR_ITEMS` · **không chữ nào về tiền** trong `choBe` · lời nhắn cho nông dân **không mang biệt danh của bé** (nông dân không nằm trong phạm vi consent) | `npm test` (bộ `de-xuat`, 37 phép) · rồi chạy thật: bé gửi ⟹ **0 `BarnTask` mới**; cha mẹ bấm một chạm ⟹ **đúng 1**; bấm hai lần ⟹ vẫn 1 và câu trả lời **tử tế** |
...
| Đổi **luật lứa mới** (`RENEW`) | hiện bị từ chối ở `assertLifecycleChoice`; UI không cho gửi | CC-B02 mới được triển khai multi-flock và quote; không reset Flock hoặc xóa Bird/Product/Health/Lot/Event; Family vẫn không RENEW | test request RENEW có 0 side effect |
...
| Đổi **chữ ở màn kết chu kỳ** | `EndOfLayChoices` + `ket-chu-ky/page.tsx` + `LIFECYCLE_STATUS_VI` | tách yêu cầu/đã nhận/đã hoàn tất; không claim kiểm dịch/sơ chế trước proof; hiển thị reason và next action | mở request pending/declined/completed, cả hai dòng và Family |
...
2. **App không đổi hiện thực.** `Barn.outside` **và `BirdGear.status → WORN/OFF`** chỉ đổi bên trong `completeTask`. Nút bấm của người dùng **tạo việc**, không đổi trạng thái: bấm "mặc yếm đỏ cho con Miu" chỉ đặt `PENDING_ON` - ngoài đời cô Lan mặc xong và chụp ảnh thì trong app mới thành `WORN`. Điều này áp dụng cho mọi tính năng "ngoài đời" thêm sau này. *(Ngoại lệ DUY NHẤT, có chủ ý: `removeGear` chuyển một `BirdGear` còn `PENDING_ON` sang `OFF`, giữ dòng và `removedAt` - cái đó chưa bao giờ tồn tại ngoài đời, nên rút lại yêu cầu không phải là "đổi hiện thực".)*
...
30. **Việc nền được đổi thứ suy ra từ LỊCH, không được khẳng định thứ chỉ ngoài đời mới biết.**
    ...
    | `→ FINISHING` (gà thịt, còn 10 ngày tới lứa) | `→ HARVESTED`/`RETIRED` - chỉ `completeTask` có proof/count/outcome của đúng request |
    ...
    CC-B01 đã khóa **RENEW** vì chưa có multi-flock. Không reset identity/ngày tiêm/sổ của đàn cũ;
    lứa mới, intake/proof và billing quote thuộc CC-B02. Gà con không xuất hiện vì một nút bấm.
...
38. **Hộp thư đi (`DomainEvent`): một đường ghi, đúng một lần, và không chở dữ liệu cấm.**
    ...
    Bảng này khác hẳn `Event` của `lib/track.ts` dù nghe giống tên (FL-D14). `Event` thường là **đo đạc** best effort; ngoại lệ 09/09: decor_installed id idem_ là biên nhận bắt buộc trong transaction, không được xóa/prune. `DomainEvent` là **lời hứa**: một bài học của đứa trẻ phải xuất hiện đúng một lần vì ngoài đời vừa có một việc thật. Neo bằng code: `tests/su-kien.test.ts` (36 phép).
...
41. **Mong muốn của bé: một câu nói, không phải một cái lệnh.**

    Epic 6 khép nửa còn thiếu của vòng lặp - bé đã học được, giờ nói được với bố mẹ. Và toàn bộ Epic đứng trên **một** câu: cái nút đứa trẻ bấm **không làm gì cả**. Neo bằng code: `tests/de-xuat.test.ts` (37 phép).

    - ⭐⭐ **Trẻ không bao giờ tạo `BarnTask`, không đổi tiền, không đổi trạng thái farm** (FL-D06/D07). `guiMongMuon` ghi đúng một dòng `ChildSuggestion` ở `PENDING` và dừng ở đó. Đây là lý do - lý do **duy nhất** - một cái nút được phép nằm trong tay đứa trẻ 5 tuổi mà không cần ai duyệt trước. **Đã đo trên máy chủ thật**: bé gửi 4 mong muốn ⟹ **0 `BarnTask` mới ở chuồng**.

    - ⭐ **Chỉ CHA MẸ biến mong muốn thành việc thật**, ở đúng một hàm (`nhoCoChuLam`), qua cổng `moMongMuon` = **sở hữu hồ sơ VÀ sở hữu chuồng** (FL-D22). Cố ý **không** thêm nghi thức xác minh lại: bố mẹ đã đăng nhập là đủ (§B của spec v1.1), và việc sinh ra không đụng tiền hay dữ liệu của trẻ. **Đã đo**: cha mẹ nhà khác bấm vào mong muốn của nhà A ⟹ từ chối, **0 việc**, dòng vẫn `PENDING`; nhà A bấm ⟹ **đúng 1 việc `FEED`**; bấm lần hai ⟹ **vẫn 1 việc**.

    - ⭐ **Catalog ĐÓNG, không một ô chữ nào.** `optionKey` phải khớp `MONG_MUON`; bảng `ChildSuggestion` **không có cột nào** nhận chữ tự do (`note`/`message`/`voice`/`media`). Một cột như thế là một đường đưa chữ chưa ai đọc của một đứa trẻ ra trước mắt người lớn, và gửi rồi thì không rút lại được. **Đã đo**: khoá bịa `XOA_HET_DU_LIEU` ⟹ từ chối, 0 dòng thêm.

    - ⭐ **Trần đếm bằng DÒNG TRONG DB, không bằng bộ đếm riêng** (FL-D23): `TRAN_CARE_WISH_TUAN = 3` **mỗi chuồng mỗi 7 ngày**, và đếm **cả cái đã bị bỏ qua** - "xin ba việc một tuần" phải là ba **lần xin**, không phải ba lần được duyệt. Đếm theo bé thì một nhà ba con là gấp ba việc cho cùng một nông dân. **Đã đo**: lần xin thứ tư bị từ chối, còn câu hỏi cho cô chú **không** bị trần đó chặn.

    - ⭐ **Lời nhắn gửi nông dân KHÔNG mang biệt danh của bé**, dù nghe ấm áp hơn hẳn. Nông dân không nằm trong phạm vi consent cha mẹ đã ký (spec §17.2), và một cái tên đã gửi đi thì không rút lại được - kể cả sau khi cha mẹ rút lời đồng ý và xoá sạch dữ liệu. Lời nhắn là **hằng số** trong catalog, không phải khuôn có chỗ điền. **Đã đo**: việc thật ghi *"Gia đình nhờ cô chú bổ sung một ít rau xanh…"*, và chuông của nông dân cũng vậy.

    - ⭐ **Báo cáo tuần KHÔNG phải bảng điểm của một đứa trẻ** (§18.2). Không phần trăm, không `x/y`, không mục tiêu, không so tuần này với tuần trước, và tuyệt đối **không so bé này với bé kia** - hai anh em đọc chung màn hình đó. Danh sách chữ cấm ở `TU_CAM_BAO_CAO`, quét cả câu sinh ra lẫn trang `/gia-dinh`. Con số duy nhất ở đó trả lời *"con đang tìm hiểu gì"*, để cha mẹ có chuyện mà hỏi con lúc ăn cơm.

    - ⭐ **MỘT cái chuông mỗi nhà mỗi tuần, và không có gì để nói thì không gửi.** Cách hiển nhiên - bắn một thông báo mỗi khi có bài mới - cho ra sáu cái chuông trong một tối đàn qua chặng, và cha mẹ sẽ tắt thông báo của cả ứng dụng trong tuần thứ hai. `cauChuongTuan` trả `null` khi rỗng và việc nền bỏ qua: một cái chuông hằng tuần nói "tuần này không có gì" là cái chuông dạy người ta thôi nhìn vào chuông. Chốt chống gửi lại là `dueNudges` cửa sổ 7 ngày - job chạy **mỗi ngày**.
    ...
    ⚠️ **Chưa chạy thật: bước gửi chuông hằng tuần.** Nó nằm trong `runDailyJobs`, mà §12 cấm chạy job đó trên DB thật - và chạy riêng hàm đó cũng sẽ gửi một dòng chuông **thật** vào tài khoản của chủ dự án rồi khoá `Nudge` 7 ngày. Phần đếm thì đã chạy thật (`/gia-dinh` vẽ đúng khối "Tuần này ở nhà mình"), `cauChuongTuan` có phép kiểm thuần, và `dueNudges`+`notify` là máy móc cũ đang chạy cho 9 lời nhắc khác. Cùng lý do với `FLOCK_STAGE_CHANGED` ở §9.38.

    Đã **thử ngược 6 ca**, mỗi ca làm đỏ đúng phép kiểm của nó: cho `guiMongMuon` tạo việc thật · bỏ `parentId` khỏi phép đổi trạng thái · biến câu báo cáo thành "Điểm số tuần này" · gửi chuông cả khi không có gì · thêm một cột chữ tự do vào `ChildSuggestion` · cho cổng lọc sẵn `status: "PENDING"`.

42. **Tắt trải nghiệm số không được dừng việc chăm một con gà.**

    Epic 7 dựng phần vận hành của pilot: tạm dừng từng suất · đường xuất dữ liệu · nhãn một chạm cho nông dân · bảng số liệu. Neo bằng code: `tests/van-hanh.test.ts` (46 phép).

    - ⭐⭐ **Tạm dừng một suất chỉ đổi ĐÚNG MỘT CỘT** (`FamilyEnrollment.status`, spec §22.3). Bốn thứ nó **không** được chạm, và mỗi cái có phép kiểm riêng soi từng tên: `Flock.lifecyclePolicy` (§9.37 - lời hứa với một đứa trẻ không hết hiệu lực vì người trực bấm một cái nút) · `BarnTask`/`Barn.workerId` (đàn gà không biết pilot là gì; cô chú sáng mai vẫn ra chuồng) · `DomainEvent` (bản ghi những gì đã xảy ra ngoài đời) · `ChildProfile.status` (tạm dừng là quyết định **vận hành** của nông trại, rút consent là quyết định về **dữ liệu** của cha mẹ - trộn hai thứ là để nông trại rút consent thay gia đình). **Đã đo trên máy chủ thật**: sau khi dừng, cả bốn **không đổi một dòng nào**.

    - ⭐ **Khu của bé đóng lại NHỜ CỔNG CŨ, không nhờ dòng nào ở Epic 7.** `moKhuCuaBe` vốn đã đòi `enrollment: { status: "ACTIVE" }` từ Epic 5 (§9.40), và materializer cũng vậy. Đây là hình dạng đúng của một kill switch: thêm một **trạng thái** đi qua cái cổng đã có, không thêm một cái cổng thứ hai để rồi hai bản lệch nhau (§11.37).

    - ⭐ **Màn hình tắt đèn thì phải có người nói vì sao.** Lúc suất dừng, nút "Vào khu của bé" biến mất - và một cánh cửa biến mất không kèm lời giải thích là cách chắc chắn nhất để một gia đình nghĩ app hỏng, hoặc tệ hơn, nghĩ **đàn gà có chuyện**. `/gia-dinh` hiện một khối vàng mang **đúng câu người trực đã chọn** ở `/admin` - cùng một chữ ở hai nơi, vì nếu không thì lúc gia đình gọi điện hỏi, người trực lại đi đoán mình đã nói gì với họ. Vì thế lý do là **catalog đóng** (`LY_DO_TAM_DUNG`), không phải ô chữ tự do: chữ ở màn quản trị là chữ chảy thẳng ra trước mặt một gia đình có con nhỏ.

    - ⭐ **Không lý do nào được nói về bệnh tật hay cái chết của con vật** (§18.4). Chuyện đó phải đi qua luồng nông trại cho người lớn kèm một playbook nội dung mà chuyên gia giáo dục, thú y và pháp lý đã duyệt - không phải rò ra qua một dòng trạng thái. Và mọi câu **đều nhắc tới đàn gà**, vì đó là câu gia đình sẽ hỏi trước tiên.

    - ⭐⭐ **Nhãn của nông dân là TUỲ CHỌN, và không bao giờ được thành bắt buộc** (§18.3 · FL-D24). Mục tiêu §4 của spec là cô chú tăng tải ≤30 phút/nông trại/tuần; một trường bắt buộc trên đường "báo xong" là thứ đứng chắn giữa một người đang đứng ngoài chuồng và việc họ vừa làm. Bộ kiểm soi **từng nhánh `nope()`** trong `completeTask` và cả điều kiện `disabled` của nút gửi. Đổi lại cô chú được gì: nhãn **thay** câu ghi chú mặc định (`note || nhan?.cau || câu chung`), nên gắn nhãn là **bớt gõ**, không phải thêm việc. Khoá lệch loại việc ⟹ **bỏ nhãn, không từ chối cả việc** - từ chối ghi nhận công của một người vì một con chip sai là hỏng thứ quan trọng để giữ một thứ chỉ để đo.

    - ⚠️ **Nhãn CHƯA đổi bài học nào**, và nói thẳng thế còn hơn để người sau tưởng nó đang làm gì đó. `chonDonVi` vẫn chọn theo `kind`; tách nội dung theo nhãn nghĩa là viết thêm biến thể bài, mà **mọi biến thể phải qua chuyên gia giáo dục** (NO-GO §23). Ở đợt này nhãn là **đầu vào để đo**: `CHECK` gộp ba mong muốn khác nhau của bé (kiểm tra nước · dọn ổ đẻ · chụp cận cảnh), nên không có nó thì báo cáo pilot không phân biệt nổi cô chú thật sự đã làm gì.

    - ⭐ **Xuất dữ liệu: chỉ đọc, và chỉ của đúng bé đó.** Đây là nửa còn thiếu của lời hứa "dữ liệu này là của bạn" - trước Epic 7 cha mẹ **xoá được nhưng không tải về được**, mà một gia đình biết cuốn album của con sẽ bốc hơi thì sẽ không xoá, họ sẽ chỉ bỏ đó. Bộ kiểm đếm: **số truy vấn trong `goiDuLieuTre` = số phép lọc theo `childId`**, vì đây là một tệp sắp rời máy chủ về máy người dùng và một dòng của nhà khác lọt vào thì không có cách nào gọi nó về. Không đường dẫn ảnh/video (ảnh là dữ liệu của **nông trại**), và tên tệp **không mang biệt danh của bé** - nó rơi vào thư mục Tải về của một cái máy có thể không chỉ mình cha mẹ dùng.

    - ⭐ **Chưa đủ dữ liệu thì nói là chưa đủ.** `tiLe` trả **`null`** khi mẫu số bằng 0, và trang vẽ dấu gạch. "0% kích hoạt" khi chưa mời nhà nào là lời nói dối kinh điển của mọi bảng số liệu tự làm: người đọc thấy đỏ rồi kết luận tính năng hỏng. Cùng tinh thần: chỉ số "trẻ tự mua" hiện là **"0 theo cấu trúc"** kèm câu *"không phải một phép đếm"* - một số 0 lấy từ phép đếm rỗng trông y hệt một số 0 lấy từ phép đếm đúng, và cái người đọc cần biết là **vì sao** nó bằng 0.

    - ⚠️ **Số phút của nông dân là ƯỚC, không phải đo.** Repo này không bấm giờ cô chú và sẽ không bấm giờ. Con số trên bảng luôn đi kèm `CANH_BAO_UOC`, và nói rõ chỗ lệch với §4.4: ngưỡng tính theo **nông trại**, còn bảng tính theo **chuồng**. Con số dùng để quyết định phải hỏi chính cô chú ở tuần phỏng vấn (Epic 8).

    **Đã đo trên máy chủ thật + DB thật** (hai gia đình + một nông dân, dữ liệu dùng-một-lần, đã dọn sạch, **51/51**), và **phép âm tính cờ tắt** đo riêng sau khi trả `.env` về nguyên trạng: `/admin/gia-dinh` ra đúng "Không tìm thấy" và **0 lần** xuất hiện mọi chuỗi của trang, khối pilot trên `/admin` biến mất.

    Đã **thử ngược 7 ca**, mỗi ca làm đỏ đúng phép kiểm của nó: cho `tamDungSuat` đổi `lifecyclePolicy` · bắt buộc gắn nhãn mới báo xong được · bỏ một phép lọc `childId` khỏi gói dữ liệu · cho `tiLe` trả 0% thay vì "chưa đủ dữ liệu" · bỏ cửa gõ-lại-mật-khẩu của đường tải về · thêm một lý do tạm dừng nói "gà ốm" · mọc một cái nút lên bảng số liệu.

43. ⭐⭐ **HÌNH CHUỒNG PHẢI NÓI ĐÚNG SỰ THẬT.** (`lib/chuong-3d.ts`, `Illustrations.tsx`, `Chuong3D.tsx`; bộ kiểm `tests/chuong-3d.test.ts`, 38 phép.)

    Cảnh chuồng nay là một khung **khối hộp** (mỗi chất liệu ba sắc độ: mặt trên sáng, mặt trước gốc, mặt bên tối) thay cho hình phẳng cũ. Nhưng phần đáng canh không phải mỹ thuật - mà là hai lời nói dối mà một hình vẽ rất dễ nói hộ:

    - ⭐⭐ **Yếm chỉ được vẽ khi nó đã ở trên con gà THẬT.** `BirdGear.status = PENDING_ON` nghĩa là chủ chuồng đã chọn màu còn cô chú **chưa ra chuồng mặc**. Vẽ cái yếm đó lên hình ngay lúc bấm là app tự khoe một việc chưa ai làm - đúng thứ cả repo này từ chối (§9.1: không ảnh thì không `DONE`). Bảng trạng thái: `WORN` ⟹ vẽ · `PENDING_OFF` ⟹ **vẫn vẽ** (chủ chuồng đã bấm tháo nhưng ngoài vườn con gà vẫn đang đeo - gỡ khỏi hình ngay là nói dối theo chiều ngược lại) · `PENDING_ON` ⟹ **không vẽ**, chỉ một dấu ⏳ trên đầu · `OFF`/không có/khoá lạ ⟹ không vẽ, không nói gì. Luật này nằm ở **đúng một hàm `ganYem`**, và bộ kiểm soi cả `src/` để không đâu khác dựng `{ yem: … }` bằng tay. ⚠️ Tầng vẽ **không được biết** `PENDING_ON`/`WORN`/`colorHex` là gì - nó chỉ được nhìn `g.yem` có hay không.
    - ⭐⭐ **Vẽ đúng số con, và số đó là đàn ĐANG SỐNG.** Trước bản này `CoopBackdrop` vẽ **cứng ba con gà** cho mọi chuồng: người nhận nuôi 6 con mở app ra đếm được 3 - mà đó là con số duy nhất trên màn hình họ kiểm chứng được bằng mắt, vì họ vừa tự tay chọn nó lúc nhận chuồng. Sai ở đúng chỗ đó thì mọi con số khác của app cũng đáng ngờ theo. Nguồn là `Bird` `status: "ALIVE"`, **không phải `flock.size`**. Quá trần vẽ (`DAN_TOI_DA = 12`) thì phải **nói ra** số thật; đàn rỗng thì sân trống kèm một câu giải thích, vì một cái sân trống không lời thì người ta tưởng app hỏng.

    Ba điều kiện kèm theo, mỗi cái vá một chỗ dễ hỏng:

    - **Tên gà không rơi vào mắt người lạ.** Chuồng trưng bày thì khách vãng lai cũng mở được (§9.5), và tên gà là do chủ chuồng đặt - có nhà để trẻ con đặt. Nên nhánh `isDemoView` truyền `soCon` (chỉ số lượng), còn `dan` (có tên) chỉ đi tới chủ chuồng, cô chú và admin. Ảnh nhỏ trong mọi danh sách cũng chỉ nhận `soCon`.
    - **Vẫn là SVG dựng ở server.** Không thư viện 3D, không `<canvas>`, không `requestAnimationFrame`/`setInterval`/`useEffect`. `Illustrations.tsx` **không có `use client`** - bắt cả 7 chỗ dùng nó (phần lớn là ảnh nhỏ 86px) gánh JavaScript vì một trang cần chạm được là trả giá sai chỗ; state nằm ở lớp bọc `Chuong3D` mỏng. Hệ toạ độ giữ **240×180** vì mỗi `BarnDecor.x/y` là bản vẽ cô chú lắp thật.
    - **Không tốn thêm một lượt chờ nào.** Hai truy vấn mới (`bird.findMany`, `birdGear.findMany`) đi **trong đúng đợt `Promise.all` sẵn có** của trang chuồng và lọc theo `barn: { slug }` - không câu nào chờ câu nào (§10). Cái `birdGear.count` cũ bị **thay thế**, nên chỉ ròng thêm một câu. Danh sách nhiều chuồng dùng **một `bird.groupBy`**.

    **Chỗ CHƯA làm, đừng tưởng đã xong:** sprite decor (cây, chậu, ổ rơm, đèn) vẫn là hình phẳng bo tròn của bản cũ, đặt cạnh khung chuồng dạng khối thì lệch phong cách - vá được nhưng là một đợt riêng, vì mỗi `svgKey` phải vẽ lại kèm `DECOR_COLORS`/`DECOR_VARIANTS`. Và **chưa đo bằng mắt trên máy thật**: cảnh chuồng mới chỉ được dựng ảnh bằng Chrome headless lúc phát triển (6 tình huống + ảnh nhỏ 86px), chưa ai mở nó trên một cái điện thoại.

    Đã **thử ngược 8 ca**, mỗi ca làm đỏ đúng phép kiểm của nó: cho `ganYem` vẽ yếm lúc `PENDING_ON` · vẽ cứng 3 con như bản cũ · lấy số con từ `flock.size` · gửi tên gà cho khách xem thử · cho truy vấn đàn lệ thuộc câu cha · thêm `use client` vào `Illustrations` · cho tầng vẽ đọc trạng thái yếm · cho mặt bên sáng hơn mặt trước. ⚠️ Ca thứ năm **lúc đầu không bắt được**: phép kiểm cắt `slice(0, 400)` nên câu truy vấn KẾ BÊN lọt vào và làm nó xanh oan - nay cắt đúng một câu bằng `cauTruyVan()`.

44. **Lifecycle request không phải physical outcome (CC-B01).** REQUESTED/ACCEPTED/IN_PROGRESS không đổi Flock/Bird terminal. Chỉ `completeTask` có proof, đối soát đúng đàn và lô được ghi outcome; request/task/lot liên kết unique. Mọi transition lifecycle khóa Barn → Flock → Request → Task và CAS. Family acceptance dùng cùng khóa Flock. Không xóa hoặc suy đoán lại LifecycleDecision cũ. RENEW đóng tới CC-B02. Cờ tắt chỉ dừng ghi lifecycle, không dừng chăm đàn.
...
46. **Trải nghiệm, quyền và minh chứng — 09/09/2026.** Family tự xác nhận sau consent/assent và cam kết; giữ enrollment riêng/LAYER/RETIRE-only, không vượt PAUSED/cờ tắt. GET không sinh bài; bài chào materialize sau POST. Scene SVG 240×180 lấy Bird ALIVE, yếm WORN/PENDING_OFF, CSS motion có dừng/reduced motion, ảnh nhỏ tĩnh. Admin chọn mẫu decor đóng; active/version CAS, không đổi hình/loại/màu hoặc giá đơn cũ. GEAR so id/status, DECOR so bản vẽ + label trước proof; mismatch rollback task/media. Nhận/bàn giao chung khóa FarmWorker; replay idemKey kiểm userId. Event biên nhận install giữ sau gỡ món. [runbook trải nghiệm](docs/engineering/CC-EXPERIENCE-20260909.md).
...
| **`notFound()` cũng trả HTTP 200** ở repo này | cùng họ với dòng trên, và lý do rõ hơn: **mọi route ở đây đều có `loading.tsx`** (bắt buộc, §8), nên Next **stream** - khung chờ bay đi cùng mã `200` trước khi phần render chạm tới `notFound()`. Đo bằng status code thì `/be/<con-nhà-khác>` trông như "mở được", và ngược lại một phép kiểm đòi `404` sẽ đỏ vĩnh viễn. Chỉ URL **không khớp route nào** mới ra `404` thật | **đo bằng NỘI DUNG**: grep biệt danh bé / tiêu đề trang / dữ liệu nhạy cảm trong HTML, phải ra **0**. Đây là cách mục Y·Z·AA của HUONG-DAN và các phép chạy thật của Epic 5–6 đo, và là cách duy nhất đúng |
...
**Cập nhật CC-B01:** mô tả MEAT/RETIRE/RENEW trước đây trong lịch sử các đợt phải đọc theo §7.11 mới. Request/proof đã tách; các gate thương mại/hold đầy đủ, shipment và multi-flock vẫn chưa thuộc slice này.
...
17. **CC-B01 (2026-09-07): RENEW đã bị khóa, đường reset/xóa Bird/Product đã bỏ.** Bản cũ reset cùng Flock và đổi mốc billing ngay lúc chủ chọn, khiến lịch sử và trạng thái đàn sai trước proof. Multi-flock/history/intake/quote thuộc CC-B02; chưa tạo Flock cycle mới trong CC-B01. Mốc billing và hóa đơn cũ được giữ nguyên.
...
42. **Đã thay 09/09 — bảo vệ lịch sử chuồng.** deleteBarn chỉ dọn chuồng chưa từng dùng: FOR UPDATE Barn rồi kiểm Flock/reservation/owner, _count mọi quan hệ và Event theo slug. Có lịch sử thì từ chối; không xóa bảng con hoặc ghi hoàn tiền ở đường này. Dùng hoàn trả/bàn giao để xử lý chuồng đã sử dụng. PostgreSQL kiểm dữ liệu còn nguyên; bỏ công thức phá hủy Đợt 12.
...
51. 🟠 **Family Learning - trụ thứ tư, mới có đúng một cái công tắc.** Đặc tả: [CHICCHIC-FAMILY-LEARNING-SPEC-v1.1-FINALIZATION.md](CHICCHIC-FAMILY-LEARNING-SPEC-v1.1-FINALIZATION.md) (đọc **trước**, thắng khi xung đột) rồi [CHICCHIC-NEXT-PLAN-FAMILY-LEARNING.md](CHICCHIC-NEXT-PLAN-FAMILY-LEARNING.md) (spec chi tiết: data model, epic, test, migration).
    ...
    **Đã đo ở Epic 5 - khu của bé** (máy chủ thật + DB thật, bộ dữ liệu dùng-một-lần, đã dọn sạch): ba trang của bé vẽ ra với **0 link sang khu người lớn, 0 SideNav, 0 chuông** (sau khi sửa lỗi lớp bọc) · ⭐ mở bài của bé A bằng đường dẫn của bé B ⟹ **`not-found`** · chưa đăng nhập ⟹ về trang đăng nhập · bấm xong kèm **một khoá bịa** ⟹ chỉ khoá thật được lưu · ⭐ **hai tab cùng bấm xong ⟹ một tab "Giỏi lắm", tab kia "Bé làm xong bài này rồi", cả hai `ok`** · bấm lại lần nữa vẫn thế · nhiệm vụ gia đình ghi vào cột riêng · cổng ra: sai mật khẩu **từ chối**, đúng thì mở, và **`reauthAt` vẫn `null`** · ⭐ **rút lời đồng ý ⟹ cả ba trang `not-found`, 0 lần xuất hiện tên bé, cả ba hành động từ chối** · ⭐ **cờ tắt ⟹ ba trang `not-found` và năm hành động từ chối** · trang cha mẹ **không còn nút chết nào** (bé đã rút: 0 nút; bé chưa gắn chuồng: 0 nút).
    ...
    - **Kill switch còn thiếu MỨC UNIT** (§22.3). Đã có mức *cả chương trình* (`FAMILY_LEARNING_ENABLED`) và mức *một suất* (`tamDungSuat`, Epic 7), nhưng chưa tắt được **một bài học / một phiên bản nội dung** khi phát hiện nó có vấn đề - hiện phải dừng cả suất của mọi nhà đang học bài đó.
    - **Nhãn của nông dân chưa đổi được nội dung bài** (§9.42): `CHECK` vẫn chỉ có một bài dù nhãn phân biệt được ba việc. Tách được thì bài của bé mới nói đúng thứ vừa xảy ra - nhưng mọi biến thể nội dung phải qua **chuyên gia giáo dục** trước (NO-GO §23).
    - **Số phút của nông dân trên `/admin/gia-dinh` là ƯỚC, không phải đo** (§9.42): repo không bấm giờ ai. Con số thật cho ngưỡng §4.4 phải hỏi chính cô chú ở tuần phỏng vấn (Epic 8).
    - **Retention tuần 6 chưa đọc được**: bảng mới có "tuần này", vì pilot chưa chạy đủ 6 tuần.
    - **Bước gửi chuông hằng tuần chưa chạy thật** (Epic 6): xem §9.41.
...
**Migration mới:** 202609080002_experience_catalog thêm active/version/CHECK trên DecorItem. `npm run db:check:catalog` là gate thứ ba của build:vercel. Backup/restore/rehearse trước khi áp; build thông thường không xác nhận DB runtime đã migrate. Không db push/seed/reset production. [runbook trải nghiệm](docs/engineering/CC-EXPERIENCE-20260909.md).
...
**Bằng chứng 09/09/2026:** 992 unit/render/source, 32 PG security/nghiệp vụ + 18 PG lifecycle đạt. HTTP mở rộng Family tự mở, catalog admin và đặt tên Unicode; kết quả cuối ở [runbook trải nghiệm](docs/engineering/CC-EXPERIENCE-20260909.md). Vercel có ba gate lifecycle/session/catalog. Vitest 4.1.11 vá GHSA-82fw-gwwq-j7x9; Vite 8 cần `oxc.jsx.runtime=automatic` để test render TSX với tsconfig Next giữ JSX.

**Bằng chứng lịch sử 08/09/2026:** 959 unit/source tests; `test:security:pg` 13 ca và `test:lifecycle:pg` 18 ca trên Postgres local riêng; `test:security:http` 37 assertions qua bản Next production (POST thật, origin, cookie, URL, HTML/API). Hai schema gate chạy trước Vercel build. Next 15.5.25, Vitest 3.2.7; audit 0 advisory tại lần kiểm tra. CI chạy Node 22 và `next typegen` trước tsc. Trên Windows không chạy generate/build cùng suite Prisma đang nạp DLL. Các số/bằng chứng của đợt cũ bên dưới là lịch sử, không thay thế UAT browser hay test dịch vụ ngoài.
...
| [tests/chuong-3d.test.ts](tests/chuong-3d.test.ts) | **§9.43 - hình chuồng dạng khối, 38 phép.** ① **Yếm chỉ vẽ khi đã mặc thật**: ⭐⭐ `PENDING_ON` ⟹ `yem === null` và **màu không rò ra** dưới bất kỳ hình dạng nào · `WORN` ⟹ vẽ, `PENDING_OFF` ⟹ **vẫn vẽ** · `OFF`/rỗng/`"worn"`/`"WORN "`/rác ⟹ đóng (fail closed) · thiếu màu ⟹ màu mặc định vẫn là hex hợp lệ · ⭐ **`yem:` chỉ được dựng trong `lib/chuong-3d.ts`**, quét cả 11 tệp liên quan · ⭐ tầng vẽ **không chứa** `PENDING_ON`/`WORN`/`BirdGear`/`gearStatus`, lớp bọc còn không được chứa `colorHex` · hai trang dựng đàn đều gọi `ganYem`. ② **Đúng số con**: n con ⟹ n chỗ đứng · quá trần ⟹ `cauSoCon` nói ra số thật · đàn rỗng ⟹ câu "trống" · `-3`/`NaN`/`Infinity`/`"6"`/`null` không làm vỡ hình · ⭐ **tất định** (không `Math.random`, server và client vẽ như nhau) · mọi con nằm trong khung · sắp theo `y` để vẽ từ xa tới gần · ⭐ **không con nào nấp sau lưng con khác** (khoảng cách >9 với mọi cỡ đàn thật) · ⭐⭐ **không trang nào lấy `soCon` từ `.size`**, ba trang danh sách đều lọc `status: "ALIVE"` · sáu trang có hình chuồng đều truyền số con · không ai chép lại `viTriDan`. ③ **Tên gà**: chưa đặt thì gọi theo vòng chân, khoảng trắng không tính là tên · ⭐ **nhánh `isDemoView` truyền `soCon`, KHÔNG truyền `dan`** · ảnh nhỏ trong 4 danh sách không mang tên gà · trang nông dân có `dan` nhưng `danGaHref={null}`. ④ **Vẫn là SVG ở server**: không `three`/`babylon`/`pixi` trong `package.json` · không `<canvas>`/`WebGL`/`requestAnimationFrame`/`setInterval`/`useEffect` · ⭐ `Illustrations.tsx` **không `use client`** · lib thuần (không `prisma`/`next/headers`/`process.env`/`react`) · ⭐ hai truy vấn mới nằm **trong** `Promise.all` và lọc theo `slug`, không `barnId` · không `include` lồng từ Bird xuống gear. ⑤ **Hình học**: mỗi khối ra 3 mặt × 4 đỉnh, không `NaN` khi sâu 0 · mặt trên sáng hơn, mặt bên tối hơn, ba mặt ba màu · màu hỏng trả nguyên vào · khung vẫn đúng 240×180. ⚠️ Đã **thử ngược 8 ca** - mỗi ca đỏ đúng chỗ; ca "truy vấn lệ thuộc câu cha" lúc đầu lọt lưới vì phép kiểm cắt lẹm sang câu bên cạnh |
| [tests/van-hanh.test.ts](tests/van-hanh.test.ts) | **§9.42 - vận hành pilot (Epic 7), 46 phép.** ① **Tạm dừng không đụng đàn**: ⭐⭐ `tamDungSuat`/`moLaiSuat` **không chứa** `lifecyclePolicy`/`flock.update`/`barnTask`/`upsertTask`/`workerId`/`childProfile`/`childConsentEvent`/`domainEvent`/`learningMoment` và **không có phép xoá nào** · `isAdmin()` → `batFamily()` → `prisma.` đúng thứ tự · so-sánh-rồi-đặt mang trạng thái cũ trong `WHERE`, `count === 0` trả `ok` · chỉ `ACTIVE` mới dừng, chỉ `PAUSED` mới mở · ⭐ khu của bé và materializer đóng **nhờ điều kiện `enrollment.status === "ACTIVE"` có sẵn** · chuông mang `body: ly.choChaMe` (không viết lại lời). ② **Lý do là catalog đóng**: khoá không trùng, ≥3 lý do · ⭐ **không câu nào** chứa `chết`/`ốm`/`bệnh`/`dịch`/`tiêu huỷ`/`mổ`/`thịt` (§18.4) · ⭐ **mọi câu đều nhắc tới "đàn"** · khoá lạ/rỗng/kiểu lạ ra một câu tử tế >20 ký tự · catalog tra trước `prisma.`. ③ **Nhãn nông dân TUỲ CHỌN**: đúng 5 khoá của §18.3 · ⭐⭐ **không nhánh `nope()` nào nhìn tới `nhan`**, nút gửi cũng không · ⭐ `note \|\| nhan?.cau \|\| câu chung` · khoá lệch loại việc và mọi kiểu rác ⟹ `null` · mọi nhãn gắn được ≥1 loại việc có thật, chữ chip ≤14 ký tự · ⭐ `CHECK` có ≥2 nhãn (lý do tồn tại của tính năng) · ⭐ vào `payload` là **khoá**, không phải câu · ⭐ **`chonDonVi` không đọc `tag`** (chặn tách nội dung mà chưa qua chuyên gia, NO-GO §23) · một cửa ghi `careTag` · đo đạc chỉ bắn trong nhánh `if (nhan)`. ④ **Xuất dữ liệu**: ⭐⭐ `lib/xuat-du-lieu.ts` **0 lời gọi ghi** · ⭐⭐ **số truy vấn = số phép lọc theo `childId`** · không `url`/`barnMedia`/`proofMediaId` · tên tệp không mang biệt danh · cổng `chaMe → daXacMinhGanDay → canParentManageChild → goiDuLieuTre` đúng thứ tự · ⭐ **không** `xoaDauXacMinh` (mà `rutConsentTre`/`xoaDuLieuTre` thì **vẫn phải có**) · nút tải nằm **trên** nút xoá. ⑤ **Bảng vận hành**: ⭐⭐ `lib/van-hanh.ts` **0 lời gọi ghi** · trang **không `<button>`, không `use client`, 0 phép ghi** · `isAdmin → batFamily → bangVanHanh` · không `nickname`/`avatarKey`/`childId` · mẫu số của tỉ lệ kích hoạt tính cả nhà đã rút. ⑥ **Chưa đủ dữ liệu thì nói thế**: ⭐ `tiLe(x, 0) === null`, `NaN`/`Infinity` ⟹ `null` · `null` ⟹ `chua-do`, không phải `chua-dat` · trang vẽ `—` · ⭐ số phút luôn kèm `CANH_BAO_UOC` và nói rõ chỗ lệch nông trại↔chuồng · ngưỡng khớp §4.4 · ⭐ "0 theo cấu trúc" kèm "không phải một phép đếm". ⑦ **Đo đạc**: bốn tên mới nằm trong danh sách đóng; props của tạm dừng chỉ `cohortKey` + khoá lý do. ⚠️ Đã **thử ngược 7 ca** - mỗi ca làm đỏ đúng phép kiểm của nó |
| [tests/de-xuat.test.ts](tests/de-xuat.test.ts) | **§9.41 - mong muốn của bé + báo cáo tuần (Epic 6), 37 phép.** ① **Mong muốn không làm gì cả**: ⭐⭐ `guiMongMuon` **không chứa** `upsertTask`/`barnTask`/`decorOrder`/`marketOrder`/`notify` · cổng `moKhuCuaBe` đứng **trước** phép ghi, có hàng rào tần suất · ⭐ phép tạo **không đặt `status`** (mặc định DB là `PENDING`) · ⭐ bảng **không có cột** `note`/`message`/`voice`/`media` · ⭐ **một cửa ghi**: mọi lời gọi `childSuggestion.*` ngoài `lib/de-xuat.ts` chỉ được là `updateMany`/`count`/`find*`. ② **Chỉ cha mẹ tạo việc**: ⭐ chỉ `nhoCoChuLam` có `upsertTask` · cổng đòi **cả** `parentId` **lẫn** `ownerId !== parentId` · ⭐ so-sánh-rồi-đặt kèm `parentId`, `count === 0` không phải lỗi · ⭐ **bấm lại thứ mình vừa trả lời ⟹ `ok`, không phải "không tìm thấy"** (cổng không được lọc sẵn `status`) · ⭐ tạo việc hỏng ⟹ quay về `PENDING` · không nút chết (`dongDan`, `outside`, `workerId`) · ⭐ **`RETIRED` không bị coi là đóng đàn** · lời nhắn nông dân là **hằng số**, không có chỗ điền. ③ **Catalog đóng**: khoá không trùng, mỗi loại ≥2 lựa chọn · ⭐ **chỉ** `CARE_WISH` mang việc và **mọi** `CARE_WISH` đều mang · ⭐ mọi `DECOR_WISH` trỏ tới slug **có thật** trong `DECOR_ITEMS` · ⭐ chữ bé đọc quét theo **chung** danh sách với nội dung bài học (`TU_CAM_NOI_DUNG`) + chữ về tiền, và ≤50 ký tự · khoá lạ bị từ chối ở mọi kiểu dữ liệu · catalog tra **trước** phép ghi. ④ **Trần việc thật**: đếm theo **chuồng** và tính cả `DECLINED` · cờ tổng đứng trước mọi `prisma.` ở cả 6 hàm · việc nền đóng mong muốn quá hạn **và không gõ cửa ai**. ⑤ **Báo cáo tuần**: ⭐ không chữ xếp hạng nào trong **mọi** câu sinh ra, không `%`, không `x/y` · tuần im ắng không có câu trách móc · trang `/gia-dinh` sạch chữ cấm · ⭐ `baoCaoTuan` **chỉ đọc** (quét theo *lời gọi* `.create`/`.update`, không theo chữ - `createdAt` chứa "create"). ⑥ **Chuông gộp**: ⭐ rỗng ⟹ `null` · một câu gộp cả ba con số · ⭐ việc nền có **đúng một** `await notify(` và `if (!cau) continue` · gửi cho **cha mẹ**, không mang biệt danh · nhà đã rút lời đồng ý không nhận gì. ⑦ **Đo đạc** chỉ mang `kind`, không `optionKey`/`nickname`/`childId`; **không có** `parent_report_viewed`. ⚠️ Đã **thử ngược 6 ca** (cho hành động của bé tạo việc thật; bỏ `parentId` khỏi phép đổi; biến báo cáo thành "Điểm số tuần này"; gửi chuông cả khi rỗng; thêm cột chữ tự do; cho cổng lọc sẵn `PENDING`) - mỗi ca làm đỏ đúng phép kiểm của nó |
...
- ⟹ Bộ này **không phủ cổng quyền, không phủ phép ghi DB, không phủ biên giới với dịch vụ ngoài**. Đừng đọc "918 passed" thành "an toàn để deploy".
