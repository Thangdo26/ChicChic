...
| **`Flock.lifecyclePolicy`** | *(chưa có nơi ghi - Epic 2)* | ⚠️ Cột này là **cam kết đã hứa với một gia đình**, không phải trạng thái. `INVITED` **không** đổi nó; chỉ lúc cha mẹ đồng ý rõ ràng mới khoá, và **không bao giờ** đảo ngược - kể cả khi họ rút consent hay xoá dữ liệu con (spec §17.4). Đọc §9.36 trước khi thêm bất kỳ đường ghi nào |
...
| [family-gates.ts](src/lib/family-gates.ts) | `coBatFamily` `FAMILY_PROGRAM_VERSION` **`allowedLifecycleChoices`** - **thuần, không Prisma/next-headers/process.env** | family.ts · **`actions.decideEndOfLay`** · `/ket-chu-ky` · tests/family.test.ts |
| [family.ts](src/lib/family.ts) | `batFamily` - cờ tổng Family Learning, **hỏng thì ĐÓNG** (§11.51) | family-admin-actions · `/admin` |
...
36. **Đàn đã hứa với một gia đình thì chỉ còn một chặng: NGHỈ HƯU.**
    ...
    - **Lời MỜI chưa khoá gì cả.** `inviteFamilyEnrollment` cố ý không đụng `Flock`. Khoá lúc mời là quyết định thay người khác về số phận một đàn gà thật, dựa trên một cái bấm của người thứ ba. Việc khoá thuộc về lúc cha mẹ đồng ý rõ ràng (Epic 2).
...
51. 🟠 **Family Learning - trụ thứ tư, mới có đúng một cái công tắc.** Đặc tả: [CHICCHIC-FAMILY-LEARNING-SPEC-v1.1-FINALIZATION.md](CHICCHIC-FAMILY-LEARNING-SPEC-v1.1-FINALIZATION.md) (đọc **trước**, thắng khi xung đột) rồi [CHICCHIC-NEXT-PLAN-FAMILY-LEARNING.md](CHICCHIC-NEXT-PLAN-FAMILY-LEARNING.md) (spec chi tiết: data model, epic, test, migration).
    ...
    **Đã đo ở Epic 1** (máy chủ thật + phiên thật + DB thật, 15 phép, đã dọn sạch): chuồng `STANDARD` vẫn `RENEW` được như xưa · đàn `FAMILY_RETIRE_ONLY` từ chối `MEAT` và `RENEW` (**0 dòng `LifecycleDecision`**, `stage` không đổi, 5 con vẫn `ALIVE`) và nhận `RETIRE` · bốn điều kiện mời đều từ chối đúng ca (gà thịt · chuồng không chủ · đàn đã `RETIRED` · chuồng không tồn tại) · chưa đăng nhập và phiên không phải admin đều bị chặn · **8 lệnh `INSERT` song song ⟹ 1 qua, 7 dính `P2002`**, bảng còn đúng 1 dòng · mời xong `Flock` **vẫn `STANDARD`** (mời ≠ khoá) · kết thúc suất rồi mời lại được, lịch sử giữ 2 dòng · xoá chuồng còn suất bị từ chối · ⭐ **cờ tắt ⟹ action từ chối VÀ `/admin` render 177KB mà không một chữ nào về Gia đình**.
    ...
    ⚠️ **Còn lại sau Epic 1:**
    - **Cha mẹ chưa có gì để bấm.** Lời mời gửi chuông dẫn về `/chuong/<slug>`, nhưng trang nhận lời mời (`/gia-dinh`) là Epic 2 - nên hiện tại một lời mời đã gửi thì **không ai nhận được**. Đừng bật cờ trên production trước Epic 2.
    - **`Flock.lifecyclePolicy` chưa có đường ghi nào.** Cột tồn tại, `decideEndOfLay` tôn trọng nó, nhưng chỉ đặt được bằng tay dưới DB. `acceptFamilyEnrollment` (Epic 2) mới là cửa thật.
...
`npm test` → [vitest.config.ts](vitest.config.ts) → `tests/*.test.ts`. **562 phép kiểm, ~3 giây**, chạy trong CI trước bước build.
...
- ⟹ Bộ này **không phủ cổng quyền, không phủ phép ghi DB, không phủ biên giới với dịch vụ ngoài**. Đừng đọc "562 passed" thành "an toàn để deploy".
