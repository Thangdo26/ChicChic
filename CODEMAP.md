...
| Đổi **luồng mua** (giỏ hàng, chốt đơn) | `market-actions.themVaoGio/boKhoiGio/chotGio` · `lib/payments.confirmMarketPaid` | ⚠️ **đọc §11.45 trước.** `chotGio` là chỗ DUY NHẤT sinh `MarketOrder.payCode`, và ba con số tiền **chụp lại tại đó** - đừng tính lại lúc hiển thị · trạng thái `OPEN` chính là cái giỏ, đừng thêm bảng giỏ thứ hai · vào giỏ **không** báo người bán (§9.8) · thêm trạng thái vào `MarketOrderStatus` thì sửa cả `market.MARKET_ORDER_VI` | bỏ 2 lô của **hai người bán khác nhau** vào một giỏ → chốt → `shipVnd` phải là **một** lần phí, `totalVnd = goodsVnd + shipVnd` · chốt hai lần → đơn thứ hai không sinh ra · tiền về → đúng **một** việc `DELIVER` mang địa chỉ |
...
`npm test` → [vitest.config.ts](vitest.config.ts) → `tests/*.test.ts`. **432 phép kiểm, ~4 giây**, chạy trong CI trước bước build.
