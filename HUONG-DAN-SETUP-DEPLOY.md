...
**Setup 09/09/2026:** SQL baseline → CC-B01 → session_scope → experience_catalog; ba schema gate thay cho suy đoán từ prisma generate. Seed decor giờ chỉ tạo mã còn thiếu, không ghi đè giá/hình/tồn admin đã quản lý. Sau khi chạy local, kiểm npm test, tsc, lint, build, PG và HTTP. [runbook trải nghiệm 09/09](docs/engineering/CC-EXPERIENCE-20260909.md) hướng dẫn migration/rollback, biến DB test và chứng cứ release.
...
> ⚠️ **R · S · T · U · V là điều kiện NO-GO của Family Learning** (W và X là nghiệm thu của
> chính Family Learning, không phải điều kiện đứng trước nó). Chương trình cho trẻ em
...
✅ **Epic 2 đã xong** - trang `/gia-dinh` đã có, nên lời mời gửi đi bây giờ **nhận được**.
Nghiệm thu phần đó ở mục **X** bên dưới.
...
22. Sau khi xoá: hồ sơ biến khỏi danh sách. Mở lại `/chuong/<slug>` → **chuồng, đàn gà,
    ...
    **vẫn chỉ có một thẻ nghỉ hưu**.
    ...
    về ba lựa chọn. Lời hứa đã hứa thì nông trại giữ.
